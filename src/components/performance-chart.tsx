  ...
  return (
    <View style={styles.wrapper}>
      <Svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none">
        ...
        <Line x1={0} y1={height * 0.14} x2={width - 30} y2={height * 0.14} stroke={colors.chartGridLine} strokeWidth={1} />
        <Line x1={0} y1={height * 0.5} x2={width - 30} y2={height * 0.5} stroke={colors.chartGridLine} strokeWidth={1} />
        <Line x1={0} y1={height * 0.86} x2={width - 30} y2={height * 0.86} stroke={colors.chartGridLine} strokeWidth={1} />
        <Path d={areaPath(values, width, height)} fill={`url(#${gradientId})`} />
        {benchmarkValues.length > 0 ? (
          <Path
            d={linePath(benchmarkValues, width, height)}
            fill="none"
            stroke={colors.chartBenchmarkLine}
            strokeWidth={1.3}
            strokeDasharray="3,3"
          />
        ) : null}
        <Path d={linePath(values, width, height)} fill="none" stroke={lineColor} strokeWidth={2} />
        <Line x1={last.x} y1={0} x2={last.x} y2={height} stroke={colors.chartCursorLine} strokeWidth={1} strokeDasharray="2,2" />
        <Circle cx={last.x} cy={last.y} r={4} fill={lineColor} />
        <SvgText x={width - 28} y={height * 0.14 + 4} fill={colors.chartAxisLabel} fontSize={9}>
          {max.toFixed(0)}
        </SvgText>
        <SvgText x={width - 28} y={height * 0.86 + 4} fill={colors.chartAxisLabel} fontSize={9}>
          {min.toFixed(0)}
        </SvgText>
      </Svg>
